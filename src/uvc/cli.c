...
void process_command(const char *cmd)
{
     ...
     * ------------------------------------------------------------------ */
        ...
        if (rw == 'W') {
        	...
        	lsc_32_write(addr, 0x01); // Write a one to config update in order to enact changes
        }
        else {
    ...
    /* ------------------------------------------------------------------
     * T: Access to the translate matrix which requires -255 to 255 values
     * ------------------------------------------------------------------ */
    else if (type == 'T') {
        uint32_t addr;
        uint16_t offset;
